...
# 取值范围0-255
saliency_map_name = 'training_data_saliency_map.mat'
saliency_map = sio.loadmat('%s/%s' % (args.data_dir, saliency_map_name))['data']
...
x = torch.Tensor(training_set).view(patch_number, img_nf, patch_size, patch_size)  # 图片切块
x = x.reshape(patch_number, img_nf, d, B, d, B).permute(0, 1, 3, 5, 2, 4)  # （p_n,i_n,B,B,d,d)
x = x.reshape(patch_number, img_nf * N, l).permute(0, 2, 1)  # (p_n,l,n*i_n)
x = x.reshape(patch_number * l, img_nf * N)
x_eig_values, x_eig_vectors = torch.linalg.eig(x.t().mm(x))  # linalg.eig计算矩阵（XTX）的奇异值
x_eig_values, x_eig_vectors = x_eig_values.real, x_eig_vectors.real
descending_k_indices = x_eig_values.sort(descending=True)[1]  # 排序
Phi_init = x_eig_vectors.t()[descending_k_indices]

model = SAUNet(N_p, B, img_nf, Phi_init)
...
my_saliency_map = DataLoader(dataset=MyDataset(saliency_map, patch_number),
                             batch_size=batch_size, shuffle=False, pin_memory=True)
...
for epoch_i in range(start_epoch + 1, end_epoch + 1):
    ...
    model.train()
    for data, saliency_map in zip(my_loader, my_saliency_map):
        x_input = data.view(-1, img_nf, patch_size, patch_size).to(device)
        saliency_map = saliency_map.view(-1, img_nf, patch_size, patch_size).to(device)
        ...
        [x_output,loss_layers_sym] = model(x_input, saliency_map, q, rand_modes)
        # loss = (x_output - x_input).abs().mean()  # L1 loss
        #loss = (x_output - x_input).pow(2).mean()  # L2 loss
        loss_discrepancy = torch.mean(torch.pow(x_output - x_input, 2))
        ...
        #loss_all = loss_discrepancy + torch.mul(gamma, loss_symmetry)
